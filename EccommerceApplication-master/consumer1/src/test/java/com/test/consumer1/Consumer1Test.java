...
public class Consumer1Test {
	...
	@Rule
	public PactProviderRuleMk2 provider = new PactProviderRuleMk2("EcommereceProvider", "localhost", 8066, this);
	...
	@Pact(consumer = "myconsumer") // will default to the provider name from mockProvider in Rule
	public RequestResponsePact defineExpectationWithState1(PactDslWithProvider builder) {

		return builder

				.given("1011")

				.uponReceiving("get product list by productid 1011")
				.path("/app/products/1011").method("GET")
				.willRespondWith().status(200)
	...
	@PactVerification(fragment = "defineExpectationWithState1")

	public void runTestWithState1() throws IOException {

		Assert.assertTrue(consumerService.getProductListById("1011").isPresent());
	}
